...
export default function StudentGrades() {
  ...
  const calculateAverage = (gradeItem: any): string => {
    try {
      // Validação 1: Verificar se o item existe
      if (!gradeItem || typeof gradeItem !== 'object') {
        return "-";
      }

      // Validação 2: Extrair valores dos bimestres
      ...
      // Validação 3: Filtrar apenas valores válidos (maiores que 0)
      const validGrades = [bim1, bim2, bim3, bim4].filter(g => g > 0 && !isNaN(g));

      // Validação 4: Se não há notas, retornar "-"
      if (validGrades.length === 0) {
        return "-";
      }

      // Validação 5: Calcular média
      const average = validGrades.reduce((a, b) => a + b, 0) / validGrades.length;

      // Validação 6: Garantir que é um número antes de usar toFixed
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {grades && grades.length > 0 && grades.map((item: any, index: number) => {
                          const displayAvg = calculateAverage(item);
                          const avgColor = getAverageColor(item);
                          
                          return (
                            <tr key={`grade-${index}-${item.id || Math.random()}`} className="hover:bg-slate-50/50 transition-colors">
                              <td className="px-6 py-5">
                                <p className="font-black text-slate-900">{item.subjectName || "N/A"}</p>
                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">{item.subjectCode || "N/A"}</p>
                              </td>
                              ...
                              <td className="px-4 py-5 text-center">
                                <span className={`text-lg font-black ${avgColor}`}>
                                  {displayAvg}
                                </span>
                              </td>
